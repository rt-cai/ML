...
class OptimizerGradientDescent(Optimizer):
    ...
    def __init__(self, optimal_tolerance=1e-2, learning_rate=1e-3, max_evals=100, verbose=False):
        """
        For A5, I got rid of function object argument. Optimizer and function object are
        theoretically orthogonal, so the fit() methods should ideally associate the two,
    ...
    def step(self):
        ...
        if self.fun_obj is None:
            raise RuntimeError("You must set the function object for the optimizer with set_fun_obj() before calling step().")
...
class OptimizerGradientDescentLineSearch(OptimizerGradientDescent):
    ...
    def __init__(self, optimal_tolerance=1e-2, gamma=1e-4, max_evals=100, verbose=False):
        super().__init__(optimal_tolerance=optimal_tolerance, learning_rate=1., max_evals=max_evals, verbose=verbose)
        self.gamma = gamma
        self.initial_learning_rate = 1e-1

    def set_learning_rate(self, learning_rate):
        raise RuntimeError("Cannot set the learning rate of a line search optimizer. Please see the documentations in optimizers.py")
...
class OptimizerGradientDescentLineSearchProximalL1(OptimizerGradientDescentLineSearch):
    ...
    def __init__(self, lammy, optimal_tolerance=1e-2, gamma=1e-4, max_evals=1000, verbose=False):
        """
        Note that lammy is passed to the optimizer, not the function object.
        """
        super().__init__(optimal_tolerance=optimal_tolerance, gamma=gamma, max_evals=max_evals, verbose=verbose)
        self.lammy = lammy
...
class OptimizerStochasticGradient(Optimizer):
    """
    A "wrapper" optimizer class, which encapsulates a child optimizer and uses
    ...
    def __init__(self, child_optimizer, learning_rate_getter, batch_size, optimal_tolerance=1e-2, max_evals=100, verbose=False):
    ...
    def reset(self):
        ...
        self.child_optimizer.reset()
        self.learning_rate_getter.reset()

    def set_fun_obj(self, fun_obj):
        super().set_fun_obj(fun_obj)
        self.child_optimizer.set_fun_obj(fun_obj)

    def set_parameters(self, parameters):
        super().set_parameters(parameters)
        self.child_optimizer.set_parameters(parameters)
    ...
    def step(self):
        ...
        batches = np.array_split(shuffled_is, n_batches)  # use np.array_split in case we can't divide evenly
        ...
        for batch_is in batches:
            ...
            self.child_optimizer.set_learning_rate(learning_rate)
            self.child_optimizer.set_fun_obj_args(X_batch, y_batch)
            self.child_optimizer.clear()

            f, g, w, break_yes = self.child_optimizer.step()  # mutates parameters
        ...
        print("Epoch {:d}\tf={:.3f}\tnorm(g)={:.3f}".format(self.num_evals, f_new, norm(g_new)))
...
class OptimizerAlternate(Optimizer):
    ...
    def __init__(self, *optimizers, switch_every=1):
        """
        The constructor takes a variable number of optimizers, which are used
        in a round-robin fashion. The optimizer index will move forward every switch_every ticks
        """
