...
from learning_rate_getters import LearningRateGetterConstant, LearningRateGetterInverse, LearningRateGetterInverseSquared, LearningRateGetterInverseSqrt
...
if __name__ == '__main__':
    ...
    if question == "1":
        ...
        utils.savefig('animals_mds_features_warmstart_no.png')

        """YOUR CODE HERE FOR Q2.1"""
        print(fun_obj.evaluate(Z, X_train)[0])
        # TODO: use PCA as a warm-start encoder using FactorlessEncoderGradientWarmStart
        fun_obj = FunObjMDSEuclidean(X_train)  # Populates the Euclidean distance matrix based on X, for fast re-use.
        optimizer = OptimizerGradientDescentLineSearch()
        ...
        fun_obj = FunObjMDSGeodesic(X_train, 2)  
        optimizer = OptimizerGradientDescentLineSearch()
        warm_start_encoder = LinearEncoderPCA(2)
        model = FactorlessEncoderGradientWarmStart(2, fun_obj, optimizer, warm_start_encoder)
        model.fit(X_train);
        ...
        utils.savefig('2.2.2.png')
        
        fun_obj = FunObjMDSGeodesic(X_train, 3)  
        optimizer = OptimizerGradientDescentLineSearch()
        warm_start_encoder = LinearEncoderPCA(2)
        model = FactorlessEncoderGradientWarmStart(2, fun_obj, optimizer, warm_start_encoder)
        model.fit(X_train);
        ...
        # TODO: Play around with the hyperparameters!

        # Assemble a neural network
        hidden_feature_dims = []  # put hidden layer dimensions to increase the number of layers in encoder
        output_dim = 2
        ...
        fun_obj = FunObjMLPLogRegL2(encoder, predictor, 1.)
